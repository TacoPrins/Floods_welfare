...
@njit
def solve(grids, par, dPi_L, iNj, mMarkov,vCoeff_C,vCoeff_NC, welfare=True):
    shape_stay                          = (grids.vTime.size, iNj, grids.vK.size, grids.vG.size, grids.vM.size, grids.vH.size, grids.vL.size, grids.vE.size)
    shape_buy                           = (grids.vTime.size, iNj, grids.vK.size, grids.vG.size, grids.vX.size,  grids.vE.size)
    shape_rent                          = (grids.vTime.size, iNj, grids.vK.size, grids.vG.size, grids.vX.size, grids.vE.size)
    ...
    type_mat = misc.DoubleGrid(np.linspace(0, grids.vK.size-1, grids.vK.size), np.linspace(0, grids.vG.size-1, grids.vG.size))
    ...
    for type_index in range(type_mat.shape[0]):
        ...
        for t_index in range(grids.vTime.size-1, -1, -1):
            ...
            for j in range(iNj-1, -1, -1):
                if j == iNj-1:
                    w_c_last,q_c_last, w_c_wf_last = continuation_value_epsilons.solve_last_period_owners_C(par, grids,  dPi_S, k_index, dP_C_prime,mortgage_size_C, welfare)
                    ...
                    w_c_vE,q_c_vE, w_c_wf_vE = continuation_value_epsilons.solve_owners_C(par, grids,  j+1, k_index, mMarkov, dPi_S, coastal_stayer_inputs,mover_inputs,dP_C_prime, mortgage_size_C, welfare)
                    w_nc_vE,q_nc_vE, w_nc_wf_vE = continuation_value_epsilons.solve_owners_NC(par, grids,  j+1, k_index, mMarkov, noncoastal_stayer_inputs,mover_inputs, dP_NC_prime,  mortgage_size_NC, welfare)
                    w_renter_vE,q_renter_vE, w_renter_wf_vE = continuation_value_epsilons.solve_renters(par, grids,  j+1, k_index, mMarkov, mover_inputs, welfare)
                ...
                for e_index in range(grids.vE.size): 
                    ...
                    vt_stay_nc[t_index,j, k_index,g_index,:,:,:,e_index], c_nc[t_index,j, k_index,g_index,:,:,:,e_index],  qt_stay_nc[t_index,j, k_index,g_index,:,:,:,e_index], b_stay_nc[t_index,j, k_index,g_index,:,:,:,e_index], vt_stay_nc_wf[t_index,j, k_index,g_index,:,:,:,e_index] = stayer_problem.solve(par, grids, j, k_index, 0, w_nc,q_nc, w_nc_wf, welfare)
                    ...
                    vt_buy_nc[t_index,j, k_index,g_index,:,e_index], qt_buy_nc[t_index,j, k_index,g_index,:,e_index], vt_buy_nc_wf[t_index,j, k_index,g_index,:,e_index] = buyer_problem_epsilons.solve(par, grids, j, k_index, 0, e_index, dP_NC, vt_stay_nc[t_index,j, k_index,g_index,:,:,:,e_index], c_nc[t_index,j, k_index,g_index,:,:,:,e_index], vt_stay_nc_wf[t_index,j, k_index,g_index,:,:,:,e_index], welfare)
    
                           
    return vt_stay_c, vt_stay_nc, vt_renter, b_stay_c, b_stay_nc, b_renter, vt_stay_c_wf, vt_stay_nc_wf, vt_renter_wf
 

@njit
def solve_initial(grids, par, iNj, mMarkov,dCoeff_C_initial,dCoeff_NC_initial, initial = True, sceptics = False, welfare=False):
    
    if sceptics:
        shape_stay  = (1,iNj, grids.vK.size, grids.vG.size, grids.vM.size, grids.vH.size, grids.vL.size, grids.vE.size)
        shape_buy   = (1,iNj, grids.vK.size, grids.vG.size, grids.vX.size,  grids.vE.size)
        shape_rent  = (1,iNj, grids.vK.size, grids.vG.size, grids.vX.size, grids.vE.size)
    else: 
        shape_stay  = (1,iNj, 1, grids.vG.size, grids.vM.size, grids.vH.size, grids.vL.size, grids.vE.size)
        shape_buy   = (1,iNj, 1, grids.vG.size, grids.vX.size,  grids.vE.size)
        shape_rent  = (1,iNj, 1, grids.vG.size, grids.vX.size, grids.vE.size)
    ...
    for k_index in range(grids.vK.size):
        if k_index == 1 and sceptics == False: 
            break
        for g_index in range(grids.vG.size):
            if initial:
                dPi_S=par.dPi_S_initial 
            else:
                dPi_S=par.vPi_S_median[-1]
                                      
            dP_C= dCoeff_C_initial
            dP_NC = dCoeff_NC_initial
            dP_C_prime = dCoeff_C_initial
            dP_NC_prime = dCoeff_NC_initial
            for h_index in range(grids.vH.size):
                for l_index in range(grids.vL.size):
                    mortgage_size_C[h_index,l_index]=grids.vL[l_index]*grids.vH[h_index]*dP_C
                    mortgage_size_NC[h_index,l_index]=grids.vL[l_index]*grids.vH[h_index]*dP_NC
            for j in range(iNj-1, -1, -1):
                ...
                for e_index in range(grids.vE.size): 
                    ...
                    vt_stay_nc[0,j, k_index,g_index,:,:,:,e_index], c_nc[0,j, k_index,g_index,:,:,:,e_index],  qt_stay_nc[0,j, k_index,g_index,:,:,:,e_index], b_stay_nc[0,j, k_index,g_index,:,:,:,e_index], vt_stay_nc_wf[0,j, k_index,g_index,:,:,:,e_index] = stayer_problem.solve(par, grids, j, k_index, 0, w_nc,q_nc, w_nc_wf, welfare)
                    vt_renter[0,j, k_index,g_index,:,e_index],qt_renter[0,j, k_index,g_index,:,e_index], b_renter[0,j, k_index,g_index,:,e_index], vt_renter_wf[0,j, k_index,g_index,:,e_index] = stayer_problem_renter.solve(par, grids, j, k_index, g_index, 0, dP_C,dP_NC, dP_C,dP_NC, w_renter,q_renter, w_renter_wf, welfare)
                    ...
                    vt_buy_nc[0,j, k_index,g_index,:,e_index], qt_buy_nc[0,j, k_index,g_index,:,e_index], vt_buy_nc_wf[0,j, k_index,g_index,:,e_index] = buyer_problem_epsilons.solve(par, grids, j, k_index, 0, e_index, dP_NC, vt_stay_nc[0,j, k_index,g_index,:,:,:,e_index], c_nc[0,j, k_index,g_index,:,:,:,e_index], vt_stay_nc_wf[0,j, k_index,g_index,:,:,:,e_index], welfare)
