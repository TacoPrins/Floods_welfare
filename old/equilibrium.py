...
def generate_pricepath(dataset, grids, sceptics, par, func, mMarkov, dPi_L, vCoeff_in_C,vCoeff_in_NC, vCoeff_C_initial, vCoeff_NC_initial, mDist0_c, mDist0_nc, mDist0_renter, rental_stock_C0, rental_stock_NC0, coastal_beq0, noncoastal_beq0, savings_beq0, method):
    ...
    for t_index in range(grids.vTime.size):  
        ...
        if t_index==0:
            ...
            guess_c = lom.LoM_C(grids,t_index, vCoeff_in_C)+(dataset[t_index-1,0]-lom.LoM_C(grids,t_index-1, vCoeff_in_C))
            guess_nc = lom.LoM_NC(grids,t_index, vCoeff_in_NC)+(dataset[t_index-1,1]-lom.LoM_NC(grids,t_index-1, vCoeff_in_NC))
        bound_c_l= 0.1
        bound_nc_l= 0.1 
       ...
       # mc_time+=end-start
        ...
        #excess_demand_C_flow, excess_demand_NC_flow, net_demand_C, net_demand_NC, investment_C, investment_NC, stock_demand_rental, rental_stock = sim.excess_demand_continuous(func, dataset[t,7], int(dataset[t,2]), int(dataset[t,3]),int(dataset[t,4]),int(dataset[t,5]),dataset[t,6], grids, par, mMarkov, dPi_S, dPi_L, iNj, mDist0_c, mDist0_nc, mDist0_renter, dataset[t,0], dataset[t,1], vt_stay_nc, vt_stay_c, vt_renter, b_c_stay, b_renter, b_nc_stay,rental_stock0, coastal_beq0, noncoastal_beq0, savings_beq0,vCoeff_in_C,vCoeff_in_NC)
...
@njit
def initialise_coefficients_initial(par, grids, method, dPi_L, iNj, mMarkov, vCoeff_C_initial, vCoeff_NC_initial, bequest_guess):
    func = True
    max_it=25
    iteration =0
    rho=0.4
    vCoeff_C_initial[1:]=0
    vCoeff_NC_initial[1:]=0
    ...
    for iteration in range(0, max_it):        
        ...
        vCoeff_in_C_initial = vCoeff_C_initial.copy()
        vCoeff_in_NC_initial= vCoeff_NC_initial.copy()
        dP_C_lom=vCoeff_in_C_initial[0]
        dP_NC_lom=vCoeff_in_NC_initial[0]
        ...
        mDist1_c, mDist1_nc, mDist1_renter, rental_stock_C, rental_stock_NC, coastal_beq, noncoastal_beq, savings_beq, _, _, _, no_beq = sim.stat_dist_finder(False, grids, par, mMarkov, dPi_L, iNj, vt_stay_c[0,], vt_stay_nc[0,], vt_renter[0,], b_stay_c[0,], b_stay_nc[0,], b_renter[0,], vCoeff_in_C_initial,vCoeff_in_NC_initial, bequest_guess)
        ...
        vCoeff_C_initial[0] = rho*dP_C_guess+(1-rho)*vCoeff_in_C_initial[0]
        vCoeff_NC_initial[0] = rho*dP_NC_guess+(1-rho)*vCoeff_in_NC_initial[0]
             
        print('Coefficients C', vCoeff_C_initial)
        print('Coefficients NC', vCoeff_NC_initial)
        
        vCoeff_C_record[iteration]=vCoeff_C_initial[0]
        vCoeff_NC_record[iteration]=vCoeff_NC_initial[0]  


        if np.all(np.abs(vCoeff_C_initial - vCoeff_in_C_initial)<0.0005*rho) and np.all(np.abs(vCoeff_NC_initial - vCoeff_in_NC_initial)<0.0005*rho):
    ...
    return dP_C_guess, dP_NC_guess, vCoeff_C_initial, vCoeff_NC_initial, mDist1_c, mDist1_nc, mDist1_renter, rental_stock_C, rental_stock_NC, coastal_beq, noncoastal_beq, savings_beq, no_beq, iteration 
...
def compute_excess_demand_pair(dP_C, dP_NC, market_data):
    ...
    excess_demand_C, excess_demand_NC, _, _, _, _, _, _,_,_ = sim.excess_demand_continuous(market_data['sceptics'], market_data['func'], market_data['grids'], market_data['par'],
        market_data['t_index'],        
        market_data['mMarkov'],  market_data['dPi_L'], market_data['iNj'],
        market_data['mDist1_c'], market_data['mDist1_nc'], market_data['mDist1_renter'],
        dP_C, dP_NC, market_data['vt_stay_c'][market_data['t_index'],], market_data['vt_stay_nc'][market_data['t_index'],], market_data['vt_renter'][market_data['t_index'],],
