...
@njit
def generate_pricepath(dataset, grids, sceptics, par, func, mMarkov, vCoeff_in_C,vCoeff_in_NC, dP_C_lag, dP_NC_lag, mDist0_c, mDist0_nc, mDist0_renter, rental_stock_C0, rental_stock_NC0, coastal_beq0, noncoastal_beq0, savings_beq0, method):
    #mc_time=0 
    #dist_time=0     
    ...
    for t_index in range(grids.vTime.size):  
       ...
       # mc_time+=end-start
        ...
        if t_index<grids.vTime.size-1:
            #start=time.perf_counter()
            mDist1_c, mDist1_nc, mDist1_renter, stock_demand_rental_C1, stock_demand_rental_NC1, coastal_beq1, noncoastal_beq1, savings_beq1, _ = sim.update_dist_continuous(sceptics, False, 0, func, grids, par, t_index, mMarkov, par.iNj, mDist0_c, mDist0_nc, mDist0_renter, dataset[t_index,0], dataset[t_index,1], vt_stay_c[t_index,], vt_stay_nc[t_index,],  vt_renter[t_index,], b_stay_c[t_index,], b_stay_nc[t_index,], b_renter[t_index,],  coastal_beq0, noncoastal_beq0, savings_beq0,vCoeff_in_C,vCoeff_in_NC, dP_C_lag, dP_NC_lag)
...
def initialise_coefficients_ss(par, grids, method, iNj, mMarkov, vCoeff_C_ss, vCoeff_NC_ss, initial = True, sceptics=True):
    ...
    for iteration in range(0, max_it):        
        ...
        mDist1_c, mDist1_nc, mDist1_renter, rental_stock_C, rental_stock_NC, coastal_beq, noncoastal_beq, savings_beq, _, _, _, no_beq = sim.stat_dist_finder(sceptics, grids, par, mMarkov, iNj, vt_stay_c[0,], vt_stay_nc[0,], vt_renter[0,], b_stay_c[0,], b_stay_nc[0,], b_renter[0,], vCoeff_in_C_ss,vCoeff_in_NC_ss, bequest_guess, initial)
