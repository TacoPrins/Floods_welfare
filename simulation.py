...
def create_path(par, iNt, dPi_S, dPi_L, initial_alpha):
    ...
    random[50] = 0
    random[86] = 0
    vSimulated_alpha = np.zeros((iNt))
    vSimulated_alpha[0] = initial_alpha
    vSimulated_eps[0] = 0
    vEps_k = np.zeros((iNt))
    ...
    for t in range(1, iNt):
        if random[t] > dPi_S:
            ...
            vSimulated_eps[t] = 1
        if t >=3:
            vEps_k[t] = par.dLambda*vEps_k[t-1] + vSimulated_eps[t-3]
        if vSimulated_eps[t] == 0:
            vSimulated_alpha[t] = (vSimulated_alpha[t-1]*(1-dPi_S))/(vSimulated_alpha[t-1]*(1-dPi_S)+(1-vSimulated_alpha[t-1])*(1-dPi_L))
        elif vSimulated_eps[t] == 1:
            vSimulated_alpha[t] = (vSimulated_alpha[t-1]*dPi_S)/(vSimulated_alpha[t-1]*dPi_S+(1-vSimulated_alpha[t-1])*dPi_L)
    
    return vSimulated_eps, vSimulated_alpha, vEps_k
...
def stat_dist_finder(sceptics, grids, par, mMarkov, dPi_L, iNj, vt_stay_c, vt_stay_nc, vt_renter, b_stay_c, b_stay_nc, b_renter, vCoeff_in_C,vCoeff_in_NC, bequest_guess):
   ...
   if sceptics==True:
       mDist0_c = np.zeros((iNj, grids.vK.size, grids.vG.size, grids.vM_sim.size, grids.vH.size, grids.vL_sim.size,grids.vE.size))
       mDist0_nc = np.zeros((iNj, grids.vK.size, grids.vG.size, grids.vM_sim.size, grids.vH.size, grids.vL_sim.size,grids.vE.size))
       mDist0_renter = np.zeros((iNj, grids.vK.size, grids.vG.size, grids.vX_sim.size, grids.vE.size))
       mDist1_c = np.zeros((iNj, grids.vK.size, grids.vG.size, grids.vM_sim.size, grids.vH.size, grids.vL_sim.size,grids.vE.size))
       mDist1_nc = np.zeros((iNj, grids.vK.size, grids.vG.size, grids.vM_sim.size, grids.vH.size, grids.vL_sim.size,grids.vE.size))
       mDist1_renter = np.zeros((iNj, grids.vK.size, grids.vG.size, grids.vX_sim.size, grids.vE.size))       
   else:
       mDist0_c = np.zeros((iNj, 1, grids.vG.size, grids.vM_sim.size, grids.vH.size, grids.vL_sim.size,grids.vE.size))
       mDist0_nc = np.zeros((iNj, 1, grids.vG.size, grids.vM_sim.size, grids.vH.size, grids.vL_sim.size,grids.vE.size))
       mDist0_renter = np.zeros((iNj, 1, grids.vG.size, grids.vX_sim.size, grids.vE.size))
       mDist1_c = np.zeros((iNj, 1, grids.vG.size, grids.vM_sim.size, grids.vH.size, grids.vL_sim.size,grids.vE.size))
    ...
    for j in range(iNj):     
        for e_index in range(grids.vE.size):
            ...
            for k_index in range(grids.vK.size):     
                if sceptics==False and k_index>0:
                    continue
