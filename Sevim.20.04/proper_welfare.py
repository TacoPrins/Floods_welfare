...
def find_expenditure_equiv(par,grids,mMarkov, vCoeff_C_initial, vCoeff_NC_initial, vCoeff_C_in, vCoeff_NC_in):
    ...
    if sceptics==False:
        ...
        k_dim=grids.vK.size        
    wf_loss = np.linspace(-0.0979591836734694, 0,50)
    ...
    coastal_beq_0 = coastal_beq
    noncoastal_beq_0 = noncoastal_beq
    savings_beq_0 = savings_beq
    for wf_idx in range(wf_loss.size):
        par.wf_wedge[0] = wf_loss[wf_idx]
        # print(par.wf_wedge[0])
        coastal_beq = coastal_beq_0
        noncoastal_beq = noncoastal_beq_0
        savings_beq = savings_beq_0
        v_owner_c_wf_SS, v_owner_nc_wf_SS, v_nonowner_wf_SS, _, _, _ = household_problem.solve_ss(grids, par, par.iNj, mMarkov,vCoeff_C_initial[0], vCoeff_NC_initial[0], initial, sceptics, welfare)
        v_nonowner_wf_expanded_SS=grid_adjust_rentshape(par,grids,v_nonowner_wf_SS)
        v_owner_c_wf_expanded_SS=grid_adjust(par,grids,v_owner_c_wf_SS)
        v_owner_nc_wf_expanded_SS=grid_adjust(par,grids,v_owner_nc_wf_SS)
        for k_index in range(k_dim):
            # if k_index == 0:
            #     print('start prints for realists')
            # else:
            #     print('start prints for optimists')
            for g_index in range(grids.vG.size):
                ...
                for t_index in range(grids.vTime.size):
                    ...
                    wf_SLR_newborns = np.sum(mDist1_renter[0,k_index, g_index, :,:]* v_nonowner_wf_expanded_SLR[t_index,0,k_index, g_index, :,:])/np.sum(mDist1_renter[0,:, :, :,:])
                    wf_SS_newborns = np.sum(mDist1_renter_SS[0,k_index, g_index, :,:]* v_nonowner_wf_expanded_SS[0,0,k_index, g_index, :,:])/np.sum(mDist1_renter_SS[0,:, :, :,:])
                    if g_index == 4 and k_index == 0:
                        print('t, with costs', t_index, -par.wf_wedge[0])
                        print('SLR:', wf_SLR_newborns)
                        print('SS,:', wf_SS_newborns)
                        
                    ce_renter_newborns[wf_idx,t_index, k_index, g_index] = wf_SS_newborns - wf_SLR_newborns 
                    if g_index == 4 and k_index == 0:
                        print('difference:', ce_renter_newborns[wf_idx,t_index, k_index, g_index])
                        print('next')
                    if t_index<grids.vTime.size-1:
                        coastal_beq=vcoastal_beq[t_index]
                        noncoastal_beq=vnoncoastal_beq[t_index]
    ...
    for k_index in range(k_dim):
        for g_index in range(grids.vG.size):
            for e_index in range(grids.vE.size):
                # if g_index < 2 and e_index == 0:
                    # print('k,g,e', k_index, g_index, e_index)
                    # print("C:", ce_C[:,k_index, g_index, e_index])
                    # print("NC:", ce_NC[:,k_index, g_index, e_index])
                    # print("renters:", ce_renter[:,k_index, g_index, e_index])
            ...
            for t_index in range(grids.vTime.size):
                # if g_index == 4:
                    # print('g,k,t', g_index,k_index,t_index)
                    # print(ce_renter_newborns[:,t_index, k_index, g_index])
                tax_equiv_newborns[t_index, k_index, g_index] = find_zero_linear(wf_loss, ce_renter_newborns[:,t_index, k_index, g_index])
                if g_index == 4 and k_index == 0:
                    
                    print('k,t', k_index,t_index)
                    print('finds', tax_equiv_newborns[t_index, k_index, g_index], 'just above index', np.searchsorted(wf_loss, tax_equiv_newborns[t_index, k_index, g_index]))
                    print('where the ce was:', ce_renter_newborns[:,t_index, k_index, g_index])
